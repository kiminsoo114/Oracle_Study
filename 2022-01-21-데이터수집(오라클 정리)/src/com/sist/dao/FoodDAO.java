...
public class FoodDAO {
	...
	/*
	 * Statement : 값+SQL동시에 전송 => 단순한 문장 , 전송값이 없는 SQL PreparedStatement : 미리 SQL문장을
	 * 전송하고 나중에 값을 채운후에 실행 => Default CallableStatement : 프로시저 호출
	 */
	...
	/*
	 * 생성자 : 객체 생성(메모리 저장)할때 호출하는 함수 역할 : 멤버변수의 초기화를 담당한다 시작할때 기능이 필요한 경우에 주로 사용
	 * ---------- 드라이버 설치 , 서버연결 , 자동로그인(ID읽기) 특성 : 리턴형이 없다 , 여러개을 만들어서 사용이 가능(오버로딩)
	 * -> 오버로딩 : 중복 메소드 정의 상속이 없다 (상속에서 예외 조건 => static , 생성자) 어느 클래스든 상관없이 사용이 가능
	 * 만든다 (public)
	 * 
	 * 소멸자 : 객체가 메모리에서 제거 할때 자동 호출 : finalize() ------------------------------ gc()
	 */
	public FoodDAO() {
		try {
			Class.forName("oracle.jdbc.driver.OracleDriver");
			/*
			 * 예외처리 : 8장 객체지향의 3대 특성 : 6장 컬렉션 , 제네릭스 : 11,12장 인터페이스 : 7장
			 */
		} catch (Exception ex) {
	...
	/*
	 * 게시판 : BoardVO ,BoardDAO 회원가입 : MemberVO , MemberDAO 영화 : MovieVO , MovieDAO
	 * 추천 : ReserveVO , ReserveDAO 기본 => 테이블 (10~20)
	 */
	// 4.1 카테고리 읽기 => SELECT
	...
	// 수집된 데이터를 오라클에 저장
	/*
	 * NO NOT NULL NUMBER CNO NUMBER POSTER NOT NULL VARCHAR2(4000) NAME NOT NULL
	 * VARCHAR2(200) SCORE NOT NULL NUMBER(2,1) ADDRESS NOT NULL VARCHAR2(500) TEL
	 * NOT NULL VARCHAR2(20) TYPE NOT NULL VARCHAR2(100) PRICE NOT NULL
	 * VARCHAR2(100) TIME VARCHAR2(200) MENU CLOB GOOD NUMBER SOSO NUMBER BAD NUMBER
	 * 
	 * // 메소드 => 매개변수 (3이상 초과하지 않는다) => 배열 , class
	 */
